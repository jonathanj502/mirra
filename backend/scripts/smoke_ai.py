...
def main():
    ...
    parser.add_argument('--chunked', action='store_true', help='Exercise cross-request voice matching with short synthetic chunks')
    ...
    for voice, sentence in [
        ...
        with _decode_audio(data, 'audio/wav') as (clip, sr):
            clips.extend([np.array(clip), np.zeros(sr, dtype=np.float32)])
    if args.chunked:
        coordinator.CHUNK_SECONDS = 15
        clips *= 2
    wav = io.BytesIO()
    ...
    if args.chunked:
        assert result['stats']['metadata']['diarization']['chunk_count'] > 1
    ...
    print(json.dumps({'status': 'passed', 'duration_seconds': round(monotonic() - started, 1),
                      ...
                      'chunks': result['stats']['metadata']['diarization']['chunk_count'],
