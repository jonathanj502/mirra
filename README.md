...
The GitHub workflow validates the app, backend, database migrations, container and an unsigned iOS simulator build. `app/eas.json` prepares preview and signed production builds; a developer account, EAS project and signing credentials are still required. There are no subscriptions, payment processing or advertising trackers.
