...
| --- | --- | --- |
| [Granola](https://www.granola.ai/) | Large editorial serif type, an asymmetrical product composition and a colorful art field create a clear focal point. | Keep the existing Inter and Instrument Serif fonts. Pair the headline with an asymmetrical recording-to-debrief composition on a soft green field. |
| [Willow](https://willowvoice.com/) | Sparse copy, generous whitespace and a high-contrast action lead into a broad product demonstration. | Keep the opening message short. Show background recording leading to one specific suggestion for a chosen goal; the existing interactive sample remains the place to explore the product. |
...
The hero should communicate attention to people and passive recording during sessions throughout the day. It must not suggest automatic or uninterrupted all-day recording. At the user's request, the people photo and both WebP sources were removed. The hero now shows two moments: “Phone down. Be here.” during a recording, followed by a debrief explicitly tied to the goal of making friends. The existing new-job follow-up supplies a concrete example of useful output. The waveform and timer are static illustrations, not live recording controls. The composition is labeled as an example and uses only HTML, CSS and SVG. Goal examples retain their existing research and context; visual polish does not justify new universal speaking rules or promises of guaranteed outcomes.
...
- The earlier redesign was reviewed at 320, 393, 768, 1024 and 1440 pixel viewports. The recording-to-debrief hero was reviewed at 320, 393, 1024 and 1440 pixels, with no horizontal overflow, clipped text or photo element. Cards stack in document order on mobile. Goal cards use native CSS subgrid to align their content, with a flex fallback.
...
- The updated hero reuses the existing SVG brand mark and adds a decorative static waveform and connecting arrow. Both people-photo WebPs and their build references remain removed. The entire sequence has an illustrative caption, and its chosen goal is visible beside the suggestion.
