...
- Expo Doctor passed all 18 checks. Local TypeScript validation and 22 passing checks cover callback-token cleanup, account isolation, offline recovery, consent, imports, both sign-out paths, deletion, archive rules, loading errors and shared color contrast. Production JavaScript export passed for web and both native platforms, including Hermes bytecode. CI now runs all-platform export as well as native compilation.
...
- All 138 backend tests passed locally (136 before reporting also passed in CI), covering auth, settings, AI consent, usage refunds, duplicate processing, pipeline output, ownership checks, export, deletion, private content reporting and error logging.
...
| Report AI output | In-app reporting sends only the reviewed text/reason/note, rejects another account's debrief, retains failed attempts and confirms only acknowledged storage. Reports appear in export and disappear with their linked conversation or account. |
...
| Content reports and optional notes | Linked to the account in Supabase; no automatic audio/transcript attachment | Private review, safety and product support |
...
- Review `content_reports` through a restricted Supabase administrative session, ordered by `created_at`. Assign a private review owner before launch, record the last reviewed timestamp, triage harmful responses, and add regression examples without customer identifiers. Never copy report content into public issues. Linked-conversation/account deletion removes the original report.
