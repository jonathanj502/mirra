...
1. Apply every migration in `supabase/migrations/`, including September 14 deletion tombstones, September 15 coaching goals and September 16 atomic recording completion, to the intended Supabase project using a migration-capable database connection. Verify the live schema, owner-scoped settings read/write policies, database value constraints, and the existing `auth.users` cascading foreign keys.
